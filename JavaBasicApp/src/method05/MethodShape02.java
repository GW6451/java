 ...
 *매개변수는 있고 반환 값이 없는 경우
  사용자로부터 값을 입력받고 입력받은 값으로 처리한 후 그 결과값을 반환하고자 할 때 주로 사용
  : 정의
  	접근 지정자 [Modifier] 반환타입 메소드명(){
  	
  		처리할 일;
  		return 결과값;
  	}
  	
  결과값을 메소드를 호출한 쪽에 반환 할때는 return이라는 키워드 사용
 */
public class MethodShape02 {
	...
	static int noParamYesReturn() {
		...
		//**반환타입이 void가 아닌 경우는 반드시 값을 반환해야 한다(return예약어로)
		//return;//[x]
		return sum;
	...
	static String getGrade() {
		...
		for(int i=0; i<jumsu.length; i++) {
			System.out.println(subjects[i]+"점수입력?");
			jumsu[i] = sc.nextInt();
			total += jumsu[i];
		}
	...
	/*문제1]
	 메소드 안에서 사용자로부터 숫자 두개(Scanner)와 산술연산자(read()사용)를 입력받는 메소드로
	 산술결과는 메소드 안에서 바로 출력하고 산술 연산자 기호를 반환하는 메소드를 정의해라
	 그리고 main메소드에서 호출하여 사용자가 입력한 연산자를 출력하라
	 */
	static char getOperator() throws IOException {
		...
		switch(op) {
			...
			case '/' : 
				System.out.printf("%d / %d = %d%n",fnum,snum,fnum/snum);
				break;
			default : System.out.printf("잘못된 연산자 기호입니다",op);
		}
		return op;
