...
/*
 저장된 성적을 처리하는 로직을 추상화
 멤버 변수] : 학생수, GradeRecord타입(배열크기는 학생수만큼)
...
public class GradeLogic {
	...
	void setNameJumsu() {
		...
		for(int i=0; i<numberOfStudents; i++) {
			System.out.printf("[%d번째 학생]%n", 1+i);
			//i번째 학생 입력
	...
	//문] 점수 순서로 1등부터 출력되도록 records배열 정렬
	void sort() {
		for(int i=0; i<numberOfStudents-1; i++) {
			for(int k=i+1; k<numberOfStudents; k++) {
				if(records[i].rank>records[k].rank) {
	...
	void print() {
		...
		//학생수만큼 반복해서 출력]
		System.out.println("==============================================");
		System.out.printf("  %-8s%-5s%-5s%-5s%-5s%-8s%s%n","NAME","KOR","ENG","MATH","SUM","AVG","RANK");
		System.out.println("==============================================");
		for(int i=0; i<numberOfStudents; i++) {
			//이름 출력]
			System.out.printf("  %-8s", records[i].name);
			//국영수 출력]
			for(int k=0; k<GradeRecord.SUBJECTS; k++) {
