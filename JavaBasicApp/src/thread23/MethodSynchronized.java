...
class MethodSyncClass{
	...
	synchronized void increase(int inc) {
		for(int i=0; i<=10; i++) {
			seed += inc;
			System.out.println(String.format("[ThredName : %s, Data : %s , i = %s]", Thread.currentThread().getName(),seed,i));
			try {
				Thread.sleep(2000);
...
public class MethodSynchronized {
	...
	public static void main(String[] args) {
		...
		//스레드는 여러 개]
		MethodSyncThread mst1 = new MethodSyncThread(msc, 2, "1st Thread");
		mst1.start();
		MethodSyncThread mst2 = new MethodSyncThread(msc, 5, "2nd Thread");
		mst2.start();
	}
