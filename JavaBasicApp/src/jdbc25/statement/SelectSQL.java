...
public class SelectSQL extends IConnectImpl {
	...
	public void execute() throws Exception {
		...
		try {
			...
			//3]쿼리문 작성
			//3-1]여러개 레코드(행:row)를 반환 하는 SELECT문
			//String sql = "SELECT * FROM emp ORDER BY hiredate DESC";
			//3-2]한 개의 레코드를 반환하거나 '선택된 레코드가 없습니다'라고 반환 하는 경우 - 주로 PK로 검색하는 경우
			...
			while(rs.next()) {//더이상 꺼내올 레코드가 없을때까지 반복
				int empno = rs.getInt(1);
				String ename = rs.getString("ename");
				String job = rs.getString("job");///칼럼명 가져올때 소문자 대문자 상관없다
				//int comm = rs.getInt("comm"); 값이 없는 경우 0반환
				String comm = rs.getString("comm") == null? "" : rs.getString("comm");
				//오라클의 DATE타입을 getDate()로 가져오는경우]
				//- 년월일만 반환
			...
			//3-2]한 개의 레코드를 반환하거나 '선택된 레코드가 없습니다'라고 반환 하는 경우
//			if(rs.next()) {
//				System.out.println(
//						String.format("%-5s%-10s%-10s%-5s%s",
//								rs.getString(1),
//								rs.getString(2),
//								rs.getString(3),
