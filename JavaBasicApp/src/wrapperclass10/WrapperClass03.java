...
public class WrapperClass03 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		System.out.println(isNumber("10A4321"));
		/*
		...
		for(int i=0;i<whiteString.length();i++) {
			if(Character.isWhitespace(
					Character.codePointAt(whiteString,i)))
				whiteCount++;
		...
		for(int i=0;i < chArray.length;i++) {
			if(Character.isLowerCase(chArray[i])) {
				...
				alphaString+=Character.toUpperCase(chArray[i]);
			}
			else {
				//1]방법-직접 출력
				//System.out.print(Character.toLowerCase(chArray[i]));
				//2]방법-소문자로 변경해서 i방에 다시 저장(원본이 변함)
				//chArray[i]=Character.toLowerCase(chArray[i]);
				//3]방법-문자들을 하나씩 String형 변수에 누적
				alphaString+=Character.toLowerCase(chArray[i]);
			}
			//2번째 방법 출력]
		...
		chArray=alphaString.toCharArray();
		...
		Short shObj = s;//오토박싱
		System.out.println(shObj.toString());
