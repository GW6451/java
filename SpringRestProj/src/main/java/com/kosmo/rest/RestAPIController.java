...
public class RestAPIController {
	...
	public String update(@PathVariable String contact_id, @RequestParam Map map, MultipartFile photo,HttpServletRequest req) throws IllegalStateException, IOException {
		String path = req.getSession().getServletContext().getRealPath("/upload");
		File file = new File(path+File.separator+photo.getOriginalFilename());
		photo.transferTo(file);		
		map.put("photo", photo.getOriginalFilename());
	...
	@CrossOrigin
	@DeleteMapping(value="/contacts", produces = "text/html; charset=UTF-8")
