...
2-2. 사용자로부터 인원수를 입력받아 인원수만큼 Person형 배열의 크기를 설정해서
'     메모리를 할당하여라
2-3. 인원수만큼 반복을 통해 이름 및 나이 주소를 입력받아 위에서 할당한 Person형 배열에
    저장하여라
​
2-4. 저장한 결과를 출력하여라
​
...
public class PersonApp01 {
	...
	public static void main(String[] args) {
		...
		person = new Person01[Integer.parseInt(numberOfPerson)];
		
		for(int i=0;i<Integer.parseInt(numberOfPerson); i++) {
			person[i] = new Person01();
		}
		
		for(int i=0;i<Integer.parseInt(numberOfPerson); i++) {
			System.out.println(i+1+"번째 사람의 이름을 입력하세요");
			person[i].name =sc.nextLine();
			System.out.println(i+1+"번째 사람의 나이를 입력하세요");
			person[i].age =Integer.parseInt(sc.nextLine());
			System.out.println(i+1+"번째 사람의 주소를 입력하세요");
			person[i].address =sc.nextLine();
		}
		
		for(int i=0; i<Integer.parseInt(numberOfPerson); i++) {
			System.out.println(i+1+"번째 사람의 정보 입니다.");
			System.out.println("====================== ");
