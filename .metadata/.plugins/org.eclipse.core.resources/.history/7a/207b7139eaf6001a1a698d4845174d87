...
public class StringConstructor {
	...
	public static void main(String[] args) {
		...
		//Stirng(int[] codePoints, int offset, int count)
		...
		//String과 StringBuffer사이는 서로 생성자 변환
		
		/*문제]Scanner클래스로 문자열을 입력받아 (nextLine()사용) char형 배열로 변환해서 출력하고
		 * 또한 입력받은 문자열을 숫자로 변환해서 2를 곱한 값을 출력해라
		 * 단, 입력받은 문자열이 숫자 형식이 아니면 숫자 형식일때까지 계속 입력 받아서 위의 결과를 출력해라	 
		 */
		Scanner sc = new Scanner(System.in);
		while(true) {
			System.out.println("숫자 형태의 문자열 입력");
