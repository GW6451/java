...
public class practiceStatement02 {
	...
	public static void main(String[] args) throws IOException {
		...
		boolean isnum1 = num1 >= '0' && num1 <= '9' || 
						 num1 >= 'A' && num1 <= 'Z' ||
						 num1 >= 'a' && num1 <= 'z';
		if(isnum1) System.out.println("알파벳 혹은 숫자");
		...
		boolean isnum2 = (num2 >= '0' && num2 <= '9') && (num2-'0') % 2 == 0;
		if(isnum2) System.out.println("2의배수");
		...
		if(isnum3) {
			if (isnum3&&ismul3) System.out.println("2의 배수");
			else System.out.println("2의 배수 아님");
		}
		else {
		...
		else if(num4 >= '0' && num4 <= '9' && num4 % 2 != 0) System.out.println("2의배수가 아님");
		...
		System.out.println("1부터 100까지 숫자중 3의배수 이거나 5의 배수인 숫자의 합(3과5의 최소공배수의 배수 제외):"+sum8);
		...
		for(int k9=1; k9<=5; k9++) {
			for(int j9=1; j9<=4; j9++) {
				if(k9==j9) System.out.printf("%-2d",1);
		...
		for(int k1=1; k1<=4; k1++) {
			for(int j1=1; j1<=4; j1++) {
				if(k1+j1 == 5) System.out.printf("%-2d",1);
				else System.out.printf("%-2d",0);
			}
			System.out.println();
		}
		...
		for(int k3=2; k3<=9; k3++) {
			for(int j3=1; j3<=9; j3++) {
				System.out.printf("%-3d%-3s%-3d%-3s%-5d",k3,'*',j3,'=',k3*j3);
			}
			System.out.println();
		}
	}///main
