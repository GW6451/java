...
public class WndowExplorer {

	public static void main(String[] args) {
		File f = new File("src/io24/node");
		System.out.println("===============================================================================");
		System.out.println(String.format("%-55s%-56s%-20s%s","이름","수정한 날짜","유형","크기"));
		System.out.println("===============================================================================");
		...
		for(File file:files) {
			...
			if(file.isFile()) {//파일인 경우
				...
				sbFile.append(String.format("%-30s%-32s%-14s%sKB%n",name,lastModified,ext+" 파일",size));
			}
			else {//디렉토리인 경우
				sbDir.append(String.format("%-30s%-32s%-14s%n",name,lastModified,"파일 폴더"));
			}
		}////////////
