...
public class AcademyLogic {
	...
	public void seperateMainMenu(int mainMenu) {
		switch(mainMenu) {
			...
			case 5 : //검색
				...
				printSubMenu2();
				//서브메뉴 입력받기
				int subMenu2 = getMenuNumber();
				if(subMenu2 == 4) break;
				//입력처리
				switch(subMenu2) {
					case 1 : searchPersonByName(); 
							break;
					case 2 : searchPersonBySubject(); 
							break;
					case 3 : searchPersonBystNumber(); 
							break;
					default : System.out.println("서브메뉴에 없는 번호");
				}
				break;
	...
	private void printSubMenu2() {
		System.out.println("=====================서브 메뉴======================");
		System.out.println("  1.이름으로 검색  2.과목으로 검색 3.학번으로 검색 4.메인메뉴로 이동 ");
		System.out.println("=================================================");
		System.out.println("-------------------서브메뉴번호 입력-------------------");
	}
	/*8] 검색	 */
	private void searchPersonByName() {
		Person findPersonByName = findPersonByName("검색");
		if(findPersonByName != null) {
			System.out.println("<"+ findPersonByName.name + "검색 결과>");
			findPersonByName.print();
		}
	...
	private void searchPersonBySubject() {
		Person findPersonBySubject = findPersonBySubject("검색");
		if(findPersonBySubject != null) {
			System.out.println("<"+ findPersonBySubject.name + "검색 결과>");
			findPersonBySubject.print();
		}
		
	}///findPersonByName(String title)
	private Person findPersonBySubject (String title) {
		System.out.println(title+"할 과목 입력");
		Scanner sc = new Scanner(System.in);
		String subject = sc.nextLine();
		for(int i=0; i< MAX_PERSON; i++) {
			if(person[i] != null && person[i] instanceof Teacher) {
				if(subject.equals(((Teacher)person[i]).subject)) {
					return person[i];
				}
			}
		///검색된 이름이 없는 경우
		...
		//검색한 이름이 없는 경우 null 반환
		System.out.println(subject+"으로(로) 검색된 정보 없음(과목)");
		return null;		
	}
	private void searchPersonBystNumber() {
		Person findPersonBystNumber = findPersonBystNumber("검색");
		if(findPersonBystNumber != null) {
			System.out.println("<"+ findPersonBystNumber.name + "검색 결과>");
			findPersonBystNumber.print();
		}
		
	}///findPersonByName(String title)
	private Person findPersonBystNumber(String title) {
		System.out.println(title+"할 학번 입력");
		Scanner sc = new Scanner(System.in);
		String name = sc.nextLine();
		for(int i=0; i< MAX_PERSON; i++) {
			if(person[i] != null && person[i] instanceof Student) {
				if(name.equals(((Student)person[i]).stNumber)) {
					return person[i];
				}
			}
		}
